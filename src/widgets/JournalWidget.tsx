  ...
  return (
    <div className="p-2 h-full w-full flex flex-col text-sm text-gray-300">
      <div className="flex justify-between items-center mb-2">
        <h3 className="font-semibold text-gray-100 text-base">Journal</h3>
      ...
      <div className="mb-2">
        <textarea
          ...
          className="w-full h-16 p-1.5 border border-gray-600 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 resize-none bg-gray-800 text-gray-100 placeholder-gray-500"
        />
        {/* Mood selector */}
                ...
                <button
                  ...
                  className={`text-base p-0.5 rounded-full transition-all ${
                    currentEntry.mood === mood
                      ? "bg-blue-900/50 transform scale-125"
                      : "hover:bg-gray-700"
                  }`}
                  title={`Feeling ${mood}`}
                >
      ...
      <div className="flex-1 overflow-y-auto space-y-1.5 pr-1">
        {entries.length > 0 ? (
          entries.map((entry) => (
            <div
              key={entry.id}
              className={`p-1.5 border border-gray-700 rounded-md text-sm ${
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                <div className="flex space-x-1">
                  <button
                    ...
                    className="text-xs text-blue-400 hover:text-blue-300 px-1 py-0.5 rounded hover:bg-gray-700"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => deleteEntry(entry.id)}
                    className="text-xs text-red-400 hover:text-red-300 px-1 py-0.5 rounded hover:bg-gray-700"
                  >
                    Delete
                  </button>
                </div>
