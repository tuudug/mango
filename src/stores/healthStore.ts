...
export const useHealthStore = create<HealthState>((set, get) => ({
  ...
  fetchHealthData: async () => {
    ...
    try {
      ...
      set({
        ...
        healthSettings: data.healthSettings || {
  ...
  goToPreviousStepsWeek: () => {
    set((state) => ({
      currentStepsWeekStart: subWeeks(state.currentStepsWeekStart, 1),
    }));
  },
  goToNextStepsWeek: () => {
    set((state) => ({
      currentStepsWeekStart: addWeeks(state.currentStepsWeekStart, 1),
    }));
  },
  ...
  updateHealthSettings: async (settingsToUpdate) => {
    ...
    if (
      ...
      useToastStore.getState().showToast({
        title: "Validation Error",
        description: "Weight goal must be null or a positive number.",
        variant: "error",
      });
      return;
    }
...
// --- Auth Subscription ---
let currentAuthSessionHealthToken =
...
useAuthStore.subscribe((state) => {
  ...
  if (newAuthSessionHealthToken && !initialHealthFetchCompletedForSession) {
    console.log(
      "[HealthStore] Auth session detected, fetching initial health data..."
    );
    fetchHealthData();
    initialHealthFetchCompletedForSession = true;
  } else if (!newAuthSessionHealthToken) {
    // User signed out
    ...
    newAuthSessionHealthToken &&
    newAuthSessionHealthToken !== currentAuthSessionHealthToken
  ) {
    console.log(
      "[HealthStore] Auth session refreshed, refetching health data..."
    );
    fetchHealthData();
  }
...
if (currentAuthSessionHealthToken && !initialHealthFetchCompletedForSession) {
  console.log(
    "[HealthStore] Initial auth session present on load, fetching health data..."
  );
