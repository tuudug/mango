...
interface TodosState {
  ...
  addTodo: (title: string, parentId?: string | null, dueDate?: string | null) => Promise<void>;
  deleteTodo: (id: string) => Promise<void>;
  toggleTodo: (id: string) => Promise<void>;
  editTodo: (id: string, newTitle: string) => Promise<void>;
  reorderTodos: (parentId: string | null, orderedIds: string[]) => Promise<void>;
...
export const useTodosStore = create<TodosState>((set, get) => ({
  ...
  _syncNestedTodos: () => set(state => ({ nestedTodos: buildTree(state.todos) })),
  ...
  fetchTodos: async () => {
    ...
    try {
      const data = await authenticatedFetch<{ todoItems: TodoItem[] }>("/api/todos", "GET", session);
      ...
      useToastStore.getState().showToast({ title: "Fetch Error", description: errorMsg, variant: "error" });
    }
  },
  ...
  addTodo: async (title, parentId, dueDate) => {
    const session = useAuthStore.getState().session;
    if (!session) {
      useToastStore.getState().showToast({ title: "You must be logged in to add todos.", variant: "error" });
      return;
    }
    ...
    try {
      ...
      set({ error: errorMsg });
      useToastStore.getState().showToast({ title: "Add Error", description: errorMsg, variant: "error" });
    }
  },
  ...
  deleteTodo: async (id) => {
    const session = useAuthStore.getState().session;
    if (!session) {
      useToastStore.getState().showToast({ title: "You must be logged in to delete todos.", variant: "error" });
      return;
    }
    ...
    findDescendants(id);
    set(state => ({ todos: state.todos.filter((item) => !itemsToRemove.has(item.id)) }));
    get()._syncNestedTodos();
    set({ error: null });

    try {
      await authenticatedFetch<void>(`/api/todos/manual/${id}`, "DELETE", session);
    } catch (e: any) {
      ...
      useToastStore.getState().showToast({ title: "Delete Error", description: errorMsg, variant: "error" });
    }
  },
  ...
  toggleTodo: async (id: string) => {
    const session = useAuthStore.getState().session;
    if (!session) {
      useToastStore.getState().showToast({ title: "You must be logged in to toggle todos.", variant: "error" });
      return;
    }
    const originalTodos = [...get().todos];
    ...
    const updatedTodo = { ...originalTodo, is_completed: !originalTodo.is_completed };
    ...
    try {
      await authenticatedFetch<void>(`/api/todos/manual/${id}/toggle`, "PUT", session);
      ...
      useToastStore.getState().showToast({ title: "Toggle Error", description: errorMsg, variant: "error" });
    } finally {
  ...
  editTodo: async (id, newTitle) => {
    ...
    if (!session || !newTitle.trim()) {
      useToastStore.getState().showToast({ title: "Login and title required to edit.", variant: "warning" });
      return;
    }
    const originalTodos = [...get().todos];
    const todoIndex = originalTodos.findIndex((t) => t.id === id);
    if (todoIndex === -1) return;
    const updatedOptimisticTodo = { ...originalTodos[todoIndex], title: newTitle.trim() };
    ...
    try {
      ...
      set(state => ({
        todos: state.todos.map((t) => (t.id === id ? { ...confirmedTodo, sourceProvider: "manual" } : t)),
      }));
      get()._syncNestedTodos();
    } catch (e: any) {
      ...
      useToastStore.getState().showToast({ title: "Edit Error", description: errorMsg, variant: "error" });
    } finally {
  ...
  reorderTodos: async (parentId, orderedIds) => {
    const session = useAuthStore.getState().session;
    if (!session) {
      useToastStore.getState().showToast({ title: "You must be logged in to reorder todos.", variant: "error" });
      return;
    }
    const originalTodos = [...get().todos];
    const newOptimisticTodos = originalTodos.map((todo) => {
    ...
    try {
      await authenticatedFetch<void>(`/api/todos/manual/reorder`, "PUT", session, { parentId, orderedIds });
      ...
      const errorMsg = e instanceof Error ? e.message : "Failed to reorder todos";
      useToastStore.getState().showToast({ title: "Reorder Error", description: errorMsg, variant: "error" });
  ...
  breakdownTodo: async (todo) => {
    ...
    const id = todo.id;
    if (!session) {
      useToastStore.getState().showToast({ title: "You must be logged in to break down todos.", variant: "error" });
      return;
    }
    set({ isLoading: true, error: null });
    ...
    try {
      ...
      const result = await authenticatedFetch<{ createdSubItems: TodoItem[] }>(
        `/api/todos/manual/${id}/breakdown`, "POST", session, requestBody
      );

      const createdCount = result.createdSubItems?.length || 0;
      if (createdCount > 0) {
        useToastStore.getState().showToast({ title: `Generated ${createdCount} sub-tasks!`, variant: "success" });
        const newItemsWithFlag = result.createdSubItems.map((item: TodoItem) => ({ ...item, isNew: true }));

        set(state => ({
            todos: [...state.todos, ...newItemsWithFlag].sort((a,b) => a.level === b.level ? a.position - b.position : a.level - b.level),
            isLoading: false
        }));
        get()._syncNestedTodos();
        ...
        setTimeout(() => {
          set(state => ({
            todos: state.todos.map((t) => newItemsWithFlag.find((newItem: TodoItem) => newItem.id === t.id) ? { ...t, isNew: false } : t)
          }));
          get()._syncNestedTodos();
        }, 1000);
      } else {
        useToastStore.getState().showToast({ title: "No sub-tasks generated.", variant: "info" });
        set({ isLoading: false });
      }
    } catch (e: any) {
      if (e instanceof ApiError && e.status === 422 && e.errorBody?.code === "NEED_MORE_CONTEXT") {
        useToastStore.getState().showToast({ title: "Task too ambiguous", description: "Please refine the task title for automatic breakdown.", variant: "warning" });
      } else {
        console.error("[TodosStore] Failed to break down todo:", e);
        const errorMsg = e instanceof Error ? e.message : "Failed to break down task";
        useToastStore.getState().showToast({ title: "Breakdown Error", description: errorMsg, variant: "error" });
      }
  ...
  moveTodo: async (id, direction) => {
    const session = useAuthStore.getState().session;
    if (!session) {
      useToastStore.getState().showToast({ title: "You must be logged in to move todos.", variant: "error" });
      return;
    }
    ...
    const siblings = currentTodos.filter((t) => t.parent_id === itemToMove.parent_id).sort((a, b) => a.position - b.position);
    ...
    const updatedTodos = currentTodos.map(todo => {
        if (todo.id === siblings[currentIndex].id) return { ...todo, position: siblings[targetIndex].position };
        if (todo.id === siblings[targetIndex].id) return { ...todo, position: siblings[currentIndex].position };
        return todo;
    });
    ...
    try {
      await authenticatedFetch<void>(`/api/todos/manual/${id}/move`, "PUT", session, { direction });
      await get().fetchTodos(); // Refetch to confirm final order
    } catch (e: any) {
      set({ todos: originalTodos, error: e.message }); // Revert
      get()._syncNestedTodos();
      console.error(`[TodosStore] Failed to move todo ${direction}:`, e);
      const errorMsg = e instanceof Error ? e.message : `Failed to move todo ${direction}`;
      useToastStore.getState().showToast({ title: "Move Error", description: errorMsg, variant: "error" });
      await get().fetchTodos(); // Refetch to get correct state
    }
  },
}));
...
useAuthStore.subscribe(
  (newSession) => {
    const { fetchTodos, _syncNestedTodos } = useTodosStore.getState();
    const newAuthSessionToken = newSession?.access_token;

    if (newAuthSessionToken && !currentAuthSessionToken) { // User signed in
      console.log("[TodosStore] Auth session detected (sign in), fetching initial todos...");
      fetchTodos();
    } else if (!newAuthSessionToken && currentAuthSessionToken) { // User signed out
      console.log("[TodosStore] Auth session removed (sign out), clearing todos...");
      useTodosStore.setState({ todos: [], nestedTodos: [], error: null, isLoading: false });
      // _syncNestedTodos(); // Not strictly necessary as todos is empty, but good practice
    } else if (newAuthSessionToken && newAuthSessionToken !== currentAuthSessionToken) { // Session refreshed or changed
      console.log("[TodosStore] Auth session refreshed/changed, fetching todos...");
      fetchTodos();
    }
...
if (currentAuthSessionToken) {
  console.log("[TodosStore] Initial auth session present on load, fetching todos...");
