# Current Progress: Paths System UI Refinement (As of 2025-04-18)

## Goal: Refine the Paths progression system concept and implement the static UI based on feedback.

## Implementation Progress:

1.  **Planning & Design:**
    - Decided to retain the linear Paths system but use a new currency, "Sparks," instead of XP for progression.
    - Defined 4 initial paths (`Optimizer`, `Sage`, `Forecaster`, `Artisan`) in `src/lib/pathsConfig.ts`, each with 5 defined rewards (including `lucide-react` icons, descriptions, Spark costs) and 5 placeholder rewards.
    - Established that most standard widgets will be available by default, with Paths unlocking advanced features, specialized widgets, and customizations.
    - Refined the UI layout for the Paths panel (`src/components/PathsPage.tsx`):
      - Panel should slide out like others, not be full-screen exclusive of the sidebar.
      - A compact section at the top displays the currently active path with a horizontal preview of its reward track (icons with tooltips).
      - An icon button (using `Sparkles` icon) is provided for allocating Sparks (modal TBD).
      - Below the active path, all available paths (including the active one, styled differently) are displayed in taller cards within a horizontally scrollable container (`custom-scrollbar` class applied).
      - Each available path card shows a vertical preview of its rewards.
      - The "Set Active Path" button is disabled for the currently active path card.
2.  **Frontend Implementation (`src/components/PathsPage.tsx`):**
    - Refactored the component to implement the new layout: top active path section, bottom horizontally scrollable available paths section.
    - Integrated `TooltipProvider`, `Tooltip`, `TooltipTrigger`, `TooltipContent` for reward icons in the active path's horizontal track.
    - Changed the "Allocate Sparks" button to a round icon button.
    - Removed the explicit "Change Path" button from the active path section (users select from the bottom list).
    - Updated the available paths section to map over `pathsConfig`, apply conditional styling and disabled state for the active path card, and add an "Active" badge.
    - Added `overflow-x-auto` and `custom-scrollbar` classes to the available paths container.
3.  **Integration (`src/components/LeftSidebar.tsx`):**
    - Modified to render `PathsPage` within the standard sliding panel structure using `getPanelClasses`.
    - Adjusted the `maxWidthClass` for the paths panel (`md:max-w-2xl`).
    - Continued passing placeholder props (`activePathId`, `unlockedRewardIds`, `currentPathProgressSparks`, `totalSparks`, `onAllocateSparks`) to `PathsPage` for static display.

## Current State:

The static UI for the Paths panel (`PathsPage.tsx`) is implemented according to the latest design specifications, using data from `pathsConfig.ts`. The core logic for state management (fetching/saving user path state, handling Spark allocation, unlocking rewards) and backend integration remains to be implemented.
...
2.  **Backend - Standardized Error Handling:**
    ...
    - **Notifications (`api/src/routes/notifications/*.ts`):** Refactored all handlers (`getNotifications`, `markAllAsRead`, `markAsRead`).

## Current State:

Frontend components `LeftSidebar` and `Dashboard` have been simplified via `PanelManagerContext` and `useDashboardLayout` hook respectively. Backend now has a standardized error handling mechanism using custom errors and middleware, applied across all major route groups. A minor workaround remains in `api/src/routes/user/updateSettings.ts` due to a persistent TypeScript error.
