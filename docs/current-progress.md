...
1.  **Phased Approach:** The Quest System implementation will be broken down into three sub-versions:
    - **v0.2.1: Foundation:** Basic database schema, UI panel (`QuestsPanel`), context (`QuestsContext`), and manual API interactions (activate, cancel, claim).
    - **v0.2.2: LLM Generation:** Integrate LLM (Gemini) to generate daily/weekly quests and criteria based on user data. Implement generation/reset logic and UI controls.
    - **v0.2.3: Automation:** Implement backend logic to automatically track user actions against criteria, update progress, and mark quests as claimable. Enhance frontend to display progress.
2.  **Manual Claiming:** Users will manually claim rewards for completed quests via a button in the UI, rather than rewards being granted automatically upon completion.
3.  **LLM Integration:** Quests and their criteria will be generated by an LLM, using user data for personalization. Robust parsing and validation of LLM output are required.
4.  **Automatic Tracking:** User actions (habit checks, steps, finance, todos, pomodoro) will automatically update the progress of relevant quest criteria via backend logic integrated into existing API endpoints.
5.  **Dedicated Panel:** A new "Quests" panel will be added to the left sidebar to manage daily and weekly quests (activating, viewing progress, claiming, cancelling, generating/resetting).
6.  **Detailed Plans:** Specific implementation details for each phase (v0.2.1, v0.2.2, v0.2.3) have been documented in separate files within the `docs/versions/` directory.
